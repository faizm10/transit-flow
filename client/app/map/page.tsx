...
export default function MapPage() {
  ...
  const [unionPearsonShapes, setUnionPearsonShapes] = useState<GeoJSON.FeatureCollection | null>(null);
  const [unionPearsonStops, setUnionPearsonStops] = useState<GeoJSON.FeatureCollection | null>(null);
  ...
  useEffect(() => {
    ...
    if (unionPearsonStops) {
      console.log("[UPX] unionPearsonStops state updated with", unionPearsonStops.features.length, "stops");
    }
  }, [unionPearsonStops]);
  ...
  const ensureUnionPearsonLayers = useCallback(() => {
    ...
    if (!map.current.getLayer("union-pearson-stops-layer")) {
      map.current.addLayer({
        ...
        paint: {
          "circle-radius": ["interpolate", ["linear"], ["zoom"], 8, 4, 12, 8, 15, 10],
          "circle-color": "#0ea5e9",
  ...
  useEffect(() => {
    ...
    if (mapContainer.current) {
      map.current = new mapboxgl.Map({
        ...
        center: [ -79.3832, 43.72 ], // GTA, Ontario coordinates
        zoom: 9, // A suitable zoom level for a city
        pitch: 0,
        bearing: 0,
      });
      ...
      map.current.on("style.load", () => {
        ...
        if (stopsData && map.current.getSource("union-pearson-stops")) {
          (map.current.getSource("union-pearson-stops") as mapboxgl.GeoJSONSource).setData(stopsData);
          console.log("[UPX] Stops source updated in style.load:", stopsData.features.length);
        } else {
          console.log("[UPX] Stops not ready at style.load");
        }
  ...
  useEffect(() => {
    if (!mapReady || !map.current || !unionPearsonStops) {
      console.log("[UPX] useEffect blocked - mapReady:", mapReady, "map.current:", !!map.current, "unionPearsonStops:", !!unionPearsonStops);
      return;
    }

    console.log("[UPX] useEffect triggered with stops data, mapReady =", mapReady);
    ...
    const updateStops = () => {
      ...
      if (source) {
        ...
        console.log("[UPX] Stops source updated successfully:", unionPearsonStops.features.length);

        // Verify rendering
        setTimeout(() => {
          if (!map.current) return;
          const rendered = map.current.queryRenderedFeatures({ layers: ["union-pearson-stops-layer"] });
  ...
  useEffect(() => {
    if (!mapReady || !map.current || !unionPearsonStops || hasFitUnionPearson.current) return;
