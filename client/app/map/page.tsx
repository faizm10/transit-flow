...
const BUS_STOP_EXCLUSIONS: Record<string, string[]> = {
  "17": [
...
const NORMALIZED_BUS_STOP_EXCLUSIONS: Record<string, Set<string>> = Object.fromEntries(
  Object.entries(BUS_STOP_EXCLUSIONS).map(([route, stopNames]) => [
    route,
    new Set(stopNames.map((name) => normalizeStopName(name))),
  ]),
);

function shouldExcludeBusStop(routeShortName: string, stopName: string): boolean {
  const routeMatch = routeShortName.trim().match(/\d{1,3}/);
  if (!routeMatch) return false;
  const route = routeMatch[0];
  const excludedStops = NORMALIZED_BUS_STOP_EXCLUSIONS[route];
  if (!excludedStops) return false;
  const normalized = normalizeStopName(stopName);
  if (excludedStops.has(normalized)) return true;

  // Handle truncated names from data or config by prefix match.
  for (const blocked of excludedStops) {
    if (blocked.startsWith(normalized) || normalized.startsWith(blocked)) {
      return true;
    }
  }
  return false;
  ...
  const selectedVariantStops = useMemo(() => {
    ...
    selectedVariantIds.forEach((variantId) => {
      ...
      stops.forEach((stop) => {
        ...
        if (
          routeType === "3" &&
          shouldExcludeBusStop(routeShortName, stop.stop_name)
        ) {
          return;
        }
        features.push({
