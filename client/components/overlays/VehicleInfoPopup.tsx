...
function ProgressBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="h-1.5 w-full bg-[var(--landing-wash)] rounded-full overflow-hidden">
      <div
...
export default function VehicleInfoPopup({
  ...
  return (
    ...
    <div className="absolute top-20 bottom-[88px] right-4 z-30 w-72 flex flex-col justify-start animate-in fade-in slide-in-from-right-3 duration-200">
      <div className="bg-[var(--landing-elevated)]/97 backdrop-blur-xl rounded-2xl border border-[var(--landing-border-2)] shadow-xl overflow-hidden flex flex-col min-h-0">
        {/* Color header */}
        ...
        <div className="p-4 overflow-y-auto flex-1">
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="font-semibold text-[var(--landing-ink)] text-sm leading-tight">
                  {lineInfo?.name ?? `Route ${trip.route_short_name}`}
                </p>
                <p className="text-xs text-[var(--landing-faint)] mt-0.5 flex items-center gap-1">
                  <span
            ...
            <button
              onClick={onClose}
              className="text-[var(--landing-faint)] hover:text-[var(--landing-muted)] transition-colors p-1 -mt-0.5 -mr-0.5"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          {/* Destination */}
          <div className="flex items-center gap-2 bg-[var(--landing-wash)] rounded-xl px-3 py-2.5 mb-3">
            <MapPin className="w-3.5 h-3.5 text-[var(--landing-faint)] flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-[var(--landing-faint)] uppercase tracking-wide font-medium">Towards</p>
              <p className="text-sm font-semibold text-[var(--landing-ink)] truncate">{trip.end_stop_name}</p>
            </div>
          </div>
          ...
          <div className="mb-3">
            ...
            <div className="flex justify-between mt-1.5 text-[10px] text-[var(--landing-faint)]">
          ...
          <div className="grid grid-cols-2 gap-2 mb-3">
            <div className="bg-[var(--landing-wash)] rounded-xl px-3 py-2">
              <p className="text-[10px] text-[var(--landing-faint)] uppercase tracking-wide font-medium mb-0.5">Departed</p>
              <p className="text-sm font-semibold text-[var(--landing-ink)] tabular-nums">{trip.start_time}</p>
            </div>
            <div className="bg-[var(--landing-wash)] rounded-xl px-3 py-2">
              <p className="text-[10px] text-[var(--landing-faint)] uppercase tracking-wide font-medium mb-0.5">Arrives</p>
              <p className="text-sm font-semibold text-[var(--landing-ink)] tabular-nums">{trip.end_time}</p>
            </div>
          </div>
          ...
          {nextStop && (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-semibold text-[var(--landing-ink)] truncate">
                  {nextStop.stop_name ?? "—"}
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-[10px] text-[var(--landing-faint)] tabular-nums">{formatSimTime(nextStop.t)}</p>
              </div>
            </div>
          )}
          ...
          {/* ETA to destination */}
          {secsToDestination > 0 && (
            <p className="text-center text-xs text-[var(--landing-faint)] mt-2.5">
              <span className="font-medium text-[var(--landing-muted)]">{secsToMinLabel(secsToDestination)}</span>
